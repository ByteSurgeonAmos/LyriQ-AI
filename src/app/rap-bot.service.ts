...
export class RapBotService {
  ...
  // 8. Update Session
  updateSession(sessionId: string, title: string, documentId: string): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const body = { title, document_id: documentId };

    return this.http.patch(`${this.baseUrl}/api/chat/history/${sessionId}/`, body, { headers });
  }
}